...
public :
	   Estudiante(){}
	   Estudiante(string nom, string ape, string dir, int tel, string fn, int ts,string cod,int id_e) : Persona(nom,ape,dir,tel,fn,ts) {
		   codigo = cod;
		   id_estudiante = id_e;
	   }
	   //metodos get(mostrar) y set(modificar)
	   ...
	   void leer(){
		   ...
		   cn.abrir_conexion();
		   if (cn.getConector()) {
			   cout << "_______Datos del estudiante______" << endl;
